...
class CreeperEnvironment(BaseEnvironment):
    def make(
        self,
        verbose: bool,
        env_path: str,
        ...
        can_destroy: bool = False,  # creeper can destroy blocks
        kill: bool = True,  # creeper is strong enough to kill player
        *args,
        **kwargs,
    ):
        if darkness:
        ...
        if can_hunt:
            ...
            killedStatKeys = ["minecraft:creeper"]
            hunt_sounds = [
...
def generate_creepers(
    ...
    while success_count < num_husks:
        ...
        charged_command = " {powered:1}"
        commands.append(
            "summon minecraft:creeper " + f"~{dx} ~{dy} ~{dz}" + charged_command
        )
